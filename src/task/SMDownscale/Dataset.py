...
from Constant import DEM_NAME, RESOLUTION_36KM, RESOLUTION_1KM, TIFF_SUFFIX, STANDARD_GRID_36KM_PATH, \
...
class InferenceDataset(Dataset):
    ...
    def _load_data(self):
        # Load DEM data
        dem_path = os.path.join(PROCESSED_DIR_PATH, DEM_NAME, RESOLUTION_1KM, f'{DEM_NAME}{TIFF_SUFFIX}')
        dem_data = read_tiff_data(dem_path).astype(np.float32)
        ...
        for date in dates:
            ...
            for data_type in [NDVI_NAME, LST_NAME, ALBEDO_NAME, PRECIPITATION_NAME]:
                file_path = os.path.join(PROCESSED_DIR_PATH, data_type, RESOLUTION_1KM, f'{date}{TIFF_SUFFIX}')
                data_dict[data_type] = read_tiff_data(file_path).astype(np.float32)
        ...
        xs = np.array(xs_list, dtype=np.float32)  # [D, H, W, C]
        ...
        # Load & Reshape position data
        _, grid_lon, grid_lat = read_tiff(STANDARD_GRID_1KM_PATH, dst_epsg_code=4326)
